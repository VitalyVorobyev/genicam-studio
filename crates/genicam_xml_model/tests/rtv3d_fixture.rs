...
fn parse_rtv3d_fixture_tolerates_groups_and_propagates_metadata() {
    ...
    // The <Group Comment="Device Information"> wrapper should be reflected in the category node.
    assert_eq!(
        device_information.comment.as_deref(),
    ...
    let device_vendor = graph
        ...
        .expect("DeviceVendorName node present");
    assert_eq!(
    ...
    assert_eq!(
        image_format_category.comment.as_deref(),
        Some("ImageFormatControl")
    );
}
