...
//!
//! Codes are **only ever appended** — never reordered. Unknown codes map to
//! `PixelFormat::Unknown`.
//!
//! | Code | PixelFormat     |
//! |------|-----------------|
...
/// Magic bytes identifying a GenICam Studio frame header.
...
impl FrameHeader {
    ...
    ///
    /// All multi-byte fields are little-endian. The returned vector has exactly
    /// [`HEADER_SIZE`] bytes.
    pub fn encode(&self) -> Vec<u8> {
        let mut buf = Vec::with_capacity(HEADER_SIZE);
        buf.extend_from_slice(&FRAME_MAGIC.to_le_bytes()); // bytes 0-1: magic
        buf.push(SUPPORTED_VERSION); // byte  2: version
        buf.push(pixel_format_to_u8(&self.pixel_format)); // byte  3: format
        buf.extend_from_slice(&self.width.to_le_bytes()); // bytes 4-7: width
        buf.extend_from_slice(&self.height.to_le_bytes()); // bytes 8-11: height
        buf.extend_from_slice(&self.seq.to_le_bytes()); // bytes 12-15: seq
        buf
    }
    ...
    /// # Errors
    ///
    /// - [`FrameHeaderError::TooShort`] — fewer than [`HEADER_SIZE`] bytes.
    /// - [`FrameHeaderError::BadMagic`] — magic mismatch.
    /// - [`FrameHeaderError::UnsupportedVersion`] — unknown version byte.
...
///
/// `PixelFormat::Unknown` always maps to `0`. Codes are only ever appended to
/// this table — never reordered. New variants must be added at the end.
...
///
/// Unknown codes (not in the table) return [`PixelFormat::Unknown`].
...
mod tests {
    ...
    fn test_encode_produces_correct_magic() {
        let hdr = make_header(PixelFormat::Mono8);
        let encoded = hdr.encode();
        // FRAME_MAGIC = 0x4746 LE → [0x46, 0x47]
    ...
    fn test_decode_unsupported_version() {
        let mut buf = make_header(PixelFormat::Mono8).encode();
        buf[2] = 2; // override version byte
        let err = FrameHeader::decode(&buf).unwrap_err();
        assert_eq!(err, FrameHeaderError::UnsupportedVersion(2));
    }

    #[test]
    fn test_decode_returns_pixel_slice() {
        let hdr = make_header(PixelFormat::Mono8);
        let mut buf = hdr.encode();
        buf.extend(vec![0xAAu8; 100]);
        let (_decoded, remaining) = FrameHeader::decode(&buf).expect("decode should succeed");
        assert_eq!(remaining.len(), 100);
    }

    #[test]
    fn test_decode_with_pixel_data_roundtrip() {
        let pixels: Vec<u8> = (0u8..=199).map(|i| i.wrapping_mul(3)).collect();
        let hdr = FrameHeader {
        ...
        let (decoded, remaining) = FrameHeader::decode(&buf).expect("decode should succeed");
        assert_eq!(decoded, hdr);
        assert_eq!(remaining, pixels.as_slice());
    }
    ...
    #[test]
    fn test_pixel_format_unknown_to_u8() {
        assert_eq!(pixel_format_to_u8(&PixelFormat::Unknown), 0);
    }
}
