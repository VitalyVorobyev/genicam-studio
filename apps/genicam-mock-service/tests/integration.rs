...
async fn test_discovery_announcement() {
    ...
    tokio::spawn(genicam_mock_service::discovery::run(
        svc.clone(),
        config.clone(),
        shutdown_rx,
    ));
...
async fn test_xml_queryable() {
    ...
    let (shutdown_tx, shutdown_rx) = tokio::sync::watch::channel(false);
    tokio::spawn(genicam_mock_service::xml_queryable::run(
        svc.clone(),
        config.clone(),
        TEST_XML.to_string(),
        shutdown_rx,
...
async fn test_bulk_read_queryable() {
    ...
    let graph = genicam_xml_model::parse_genicam_xml(TEST_XML).expect("parse XML");
    let store = Arc::new(NodeStore::from_graph(&graph, &config));

    let (shutdown_tx, shutdown_rx) = tokio::sync::watch::channel(false);
    tokio::spawn(genicam_mock_service::nodes::run_bulk_read_queryable(
        svc.clone(),
        config.clone(),
        store.clone(),
        shutdown_rx,
...
async fn test_node_set_queryable() {
    ...
    let (shutdown_tx, shutdown_rx) = tokio::sync::watch::channel(false);
    tokio::spawn(genicam_mock_service::nodes::run_set_queryable(
        svc.clone(),
        config.clone(),
        store.clone(),
        shutdown_rx,
