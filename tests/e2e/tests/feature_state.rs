...
async fn gain_auto_round_trips_via_feature_state() {
    ...
    assert_eq!(initial.kind, "Enumeration", "GainAuto should be an Enumeration");
...
async fn acquisition_start_flips_status() {
    ...
    let sub = harness
        .session()
        .declare_subscriber(viva_zenoh_api::keys::acquisition_status(harness.device_id()))
        .await
    ...
    assert!(saw_active, "expected AcquisitionStatus.active=true after Start");
